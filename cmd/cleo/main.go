...
import (
	...
	workflowcost "github.com/kaka-ruto/cleo/internal/workflow/cost"
	workflowpr "github.com/kaka-ruto/cleo/internal/workflow/pr"
	workflowqa "github.com/kaka-ruto/cleo/internal/workflow/qa"
	workflowrelease "github.com/kaka-ruto/cleo/internal/workflow/release"
	workflowsetup "github.com/kaka-ruto/cleo/internal/workflow/setup"
	workflowskill "github.com/kaka-ruto/cleo/internal/workflow/skill"
	workflowtask "github.com/kaka-ruto/cleo/internal/workflow/task"
	workflowupdate "github.com/kaka-ruto/cleo/internal/workflow/update"
)
...
func runCost(args []string) int {
	...
	if args[2] == "help" || args[2] == "--help" || args[2] == "-h" {
		if len(args) > 3 && !help.PrintCostCommand(os.Stdout, args[3]) {
			fmt.Fprintf(os.Stderr, "unknown cost command: %s\n\n", args[3])
			help.PrintCost(os.Stderr)
			return 2
		}
		if len(args) <= 3 {
			help.PrintCost(os.Stdout)
...
func runQA(args []string) int {
	...
	cmd := workflowqa.New(workflowqa.NewAdapter(store, state.RepoKey(cfg), cfg))
	if err := cmd.Execute(args[2], args[3:]); err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
